...
\begin{itemize}
    ...
    \item Ortogonal matris $A \equiv$ $A$ Inverterbar med $A^{-1}=A^T$, $AA^T=A^TA=I$, % TODO: är del två sann?
...
\textbf{Vad kan man säga från ett element i $e^{At}$?}
\begin{itemize}
    \item Om t finns i elementet så är den ej diagonaliserbar
    \item Om elementet har en term som är konstant så är den ej inverterbar då konstanten är $C*e^{0t}$ så $0$ är ett egenvärde. ($det(A)=0$)
...
% TODO: Add förel 12 video 12 11:25s
...
\subsection{Kontrollera efter uppgift}
